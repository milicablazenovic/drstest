...
const BuyCryptoPage: React.FC = () => {
    ...
    const buyCrypto = async () => {
        ...
        try{
            const response = await httpClient.post("//127.0.0.1:5000/add_transaction", {
                name, symbol, 
                'amount': Number(amount)*100, type, 'time_transacted': transactionDate,
    ...
    return ( 
        <div className="auth">
            ...
            <form>
                ...
                <input type="text" required value={name} placeholder="Enter the Cryptocurrency's name: "
                    onChange={(e) => setName(e.target.value)}></input><br/>
                <input type="text" required value={symbol} placeholder="Enter the Crypto symbol: "
                    onChange={(e) => setSymbol(e.target.value)}></input><br/>
                <input type="number" required value={amount} placeholder="Amount: "
                    onChange={(e) => setAmount(e.target.value)}></input><br/>
                <input type="number" required value={type} placeholder="Enter 1 for Buying / 2 for Selling Crypto :"
                    onChange={(e) => setType(e.target.value)}></input><br/>
                <ReactDatePicker dateFormat="dd/MM/yyyy" required selected={transactionDate}
                    onChange={(e) => setTransactionDate(e)}></ReactDatePicker><br/>
                {/* <input type="time" required value={transactionCreated}></input> */}
                <button type="button" onClick={() => buyCrypto()}>Submit</button>
            </form>
        </div>
    );
}
